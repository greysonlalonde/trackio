...
Each version also has a **Lineage** section showing an interactive graph of the version's neighborhood: artifact versions and runs as nodes, with arrows from a run to the artifacts it logged and from an artifact to the runs that used it. The selected version is highlighted, and the view starts two hops out — `+N` badges on the edge of the graph reveal more of the chain one hop at a time. Drag to pan, use Ctrl/Cmd + scroll (or the +/− buttons) to zoom, and click any node to open a summary with a link to that run or artifact version.
